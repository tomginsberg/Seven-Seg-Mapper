...
# set your mapping of which displays map to which output varibale names in your code
...
def get_loc(segnum, varname, invert=True):
    ...
    return "\n".join(["set_location_assignment {} -to {}[{}]".format(segs[segnum][i], varname, int(
        6 * invert - 2 * (invert - 1 / 2) * i)) for i in range(7)])
